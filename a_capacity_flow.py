"""
This script can be used to calculate wind energy capacity inflow, stock and outflow

This script contains:
- inflow-driven dMFA for historical onshore wind energy
- stock-driven dMFA for future onshore and offshore wind energy
...
def get_capacity_data_from_excel(excel_path):
    ...
    stock_future_onshore = {
        'Gcam': df_onshore['on_future_capacity_stock'].dropna().to_list()[0: 7],
        'GNZ': df_onshore['on_future_capacity_stock_1'].dropna().to_list()[0: 51],
    }
    stock_future_offshore = {
        'Gcam': df_offshore['off_future_capacity'].dropna().to_list()[0: 7],
        'GNZ': df_offshore['off_future_capacity_1'].dropna().to_list()[0: 51],
    }
    # extract the historical/future year data
    def process_years(df_col, length):
        years = df_col.dropna().to_list()[0: length] if length else df_col.dropna().to_list()
        years = [int(year) for year in years]
        return years
    years_future_onshore, years_future_offshore, years_history_onshore = [
        process_years(df_col, length) for df_col, length in zip(
...
class CapacityFlow:
    ...
    def __init__(self, excel_path:str="input_data/Wind_data.xls"):
        ...
        (self.stock_future_onshore, self.years_future_onshore, self.years_history_onshore,
         ...
         self.historical_lifetime, self.future_lifetime) = \
            get_capacity_data_from_excel(excel_path)
        # interpolate the data from 2020 to 2024 using ARIMA
        self.inflow_forecast = self.interp_20_24_with_arima()

    def interp_20_24_with_arima(self):
        """Interpolate the data from 2020 to 2024 using ARIMA"""
        model = ARIMA(pd.Series(self.inflow_history_onshore), order=(0, 2, 1))
        model_fit = model.fit()
    ...
    def interp_annual_for_future(self, 
        ...
        if capacity_scenario == 'GNZ':
            ...
            stock_future_onshore = np.interp(years_future_onshore_annual[4:], years_future_onshore, stock_future_onshore)
    ...
    def update_capacity(self, capacity_scenario:str='Gcam'):
        """Update the capacity data with the interpolated data"""

        if capacity_scenario == 'Gcam':
            update_years = [2020]
        elif capacity_scenario == 'GNZ':
            update_years = [2020, 2021]

        inflow_history_onshore = np.append(self.inflow_history_onshore, self.inflow_forecast[0: len(update_years)])
        years_history_onshore = np.append(self.years_history_onshore, update_years)
        years_future_onshore = self.years_future_onshore[len(update_years):]
        stock_future_onshore = self.stock_future_onshore[capacity_scenario][len(update_years):]
        stock_future_offshore = self.stock_future_offshore[capacity_scenario]
        years_future_offshore = self.years_future_offshore

        years_future_onshore_annual = np.arange(2020 + len(update_years), years_future_onshore[-1] + 1)
        years_future_offshore_annual = np.arange(years_future_offshore[0], years_future_offshore[-1] + 1)

        return stock_future_onshore, years_future_onshore, years_history_onshore, inflow_history_onshore, \
            stock_future_offshore, years_future_offshore, years_future_onshore_annual, years_future_offshore_annual
    ...
    def plot(self, tech_scenario:int=0):
        ...
        years_future = np.arange(2019, 2051)
        ...
        for line_type, line_width, capacity_scenario in zip(['-', '--'], [2, 1], ['Gcam', 'GNZ']):
            inflow_onshore, inflow_future_onshore, stock_onshore, outflow_onshore, inflow_offshore, outflow_offshore, stock_offshore, \
                ...
                years_onshore, years_offshore = \
                self(tech_scenario=tech_scenario, capacity_scenario=capacity_scenario)
            # plot onshore data
            axs[0, 0].plot(years_future, inflow_onshore[len(years_history) - 1: ], label=capacity_scenario, color=color_dict[capacity_scenario], linestyle=line_type, linewidth=line_width)
            axs[0, 1].plot(years_future, stock_onshore[len(years_history) - 1: ], label=capacity_scenario, color=color_dict[capacity_scenario], linestyle=line_type, linewidth=line_width)
            axs[0, 2].plot(years_future, outflow_onshore[len(years_history) - 1: ], label=capacity_scenario, color=color_dict[capacity_scenario], linestyle=line_type, linewidth=line_width)
            # plot offshore data
    ...
    def __call__(self, tech_scenario:int=0, capacity_scenario:str='Gcam'):
        ...
        # update the future stock if it's Gcam
        if capacity_scenario == 'Gcam':
            interp_21_24 = np.interp([2021, 2022, 2023, 2024], [2020, 2025], [stock_history_onshore[-1], stock_future_onshore[0]])
            stock_future_onshore = np.concatenate((interp_21_24, stock_future_onshore))
