...
if __name__ == '__main__':
    ...
    for strategy in results:
        ...
        virgin_material = {}
        for m in materials:
            diff = np.asarray([mass_by_year_rep[i][m] for i in range(2020, 2051)]) - result[m][:, 0]
        ...
        df.to_csv('results/offshore_EoL/offshore_{}_sum_{}_{}.csv'.format(strategy, tp, scen))

        for m in result.keys():
            result[m][result[m] < 0] = 0
            df = pd.DataFrame(result[m], columns=proc_methods, index=year)
            df.to_csv('results/offshore_EoL/offshore_{}_{}_{}_{}.csv'.format(strategy, m, tp, scen))