"""
This script is used to calculate onshore wind turbine material production environmental impact
...
# calculate the onshore wind turbine material production environmental impact
def get_onshore_env_impact(tp, scen):
    ...
    for si, sn in enumerate(strategy_list):
        ...
        for i, m in enumerate(mass_by_year_rep[1993]):
            ...
            for k in range(len(recy_list)):
                if recy_list[k] > mass[k]:
                    recy_list[k + 1] += recy_list[k] - mass[k]
                    recy_list[k] = mass[k]
            ...
            if m_conv != 'EE':
                en_consume_by_mat[m_conv] = np.array(inflow) * coef if m_conv not in en_consume_by_mat else en_consume_by_mat[m_conv] + np.array(inflow) * coef
            
            coef = env_impact['Energy_saved (MJ kg-1)'][m_conv]
            en_save += np.array(recy_list) * coef
            if m_conv != 'EE':
                en_save_by_mat[m_conv] = np.array(recy_list) * coef if m_conv not in en_save_by_mat else en_save_by_mat[m_conv] + np.array(recy_list)* coef
            
            coef = env_impact['CO2_emission (kg)'][m_conv]
            co2_consume += np.array(inflow) * coef
            if m_conv != 'EE':
                co2_consume_by_mat[m_conv] = np.array(inflow) * coef if m_conv not in co2_consume_by_mat else co2_consume_by_mat[m_conv] + np.array(inflow) * coef
            
            coef = env_impact['CO2_reduction(kg)'][m_conv]
            co2_save += np.array(recy_list) * coef
            if m_conv != 'EE':
                co2_save_by_mat[m_conv] = np.array(recy_list) * coef if m_conv not in co2_save_by_mat else co2_save_by_mat[m_conv] + np.array(recy_list) * coef
        ...
        df.to_csv('results/onshore_env_impact_{}_{}_{}.csv'.format(sn, tp, scen))

        # energy comsumption and saving/reduction
        ...
        # climate change impact and saving/reduction
        ...
        ax.set_xticks(np.arange(len(time_agg)) - bar_width/2)
        ax.set_xticklabels(time_agg)
        ax.set_ylabel('t CO2 eq')
        ax.legend()
        
    # save en_axes to figure
    en_fig.tight_layout()
    en_fig.savefig('save_figs/onshore_energy_{}_{}.png'.format(tp, scen))
    
    co2_fig.tight_layout()
    co2_fig.savefig('save_figs/onshore_co2_{}_{}.png'.format(tp, scen))
    plt.close()
    
    export_results['time_agg'] = time_agg
...
if __name__ == '__main__':
    ...
    tp = args.tp
    scen = args.scen
    get_onshore_env_impact(tp, scen)
